...
function devicePicker(){
  ...
  return `<div class="picker-backdrop"><section class="device-picker" role="dialog" aria-label="Supported devices"><div class="picker-handle"></div><div class="picker-title"><strong>Check supported devices</strong><button data-action="close-picker" aria-label="Close">×</button></div><p>Find the device that will use this eSIM.</p><input id="picker-search" class="search" value="${pickerQuery}" placeholder="Search iPhone, Samsung, Pixel…" autofocus />${result}</section></div>`;
}
...
function render(){
  ...
  else if(state === 'search') screen.innerHTML = shell('Check compatibility', `<button class="back" data-action="device-choice">← Change device</button><p class="screen-label">Another device</p><h2>Find the device you’ll use</h2><input class="search" placeholder="Search iPhone, Samsung, Pixel…" autofocus /><div class="models"><button class="model" data-action="compatible">iPhone 13</button><button class="model" data-action="compatible">Samsung Galaxy S24</button><button class="model" data-action="incompatible">Samsung Galaxy A14</button><button class="model" data-action="compatible">Google Pixel 9</button></div>`);
