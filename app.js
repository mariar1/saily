...
function render(){
  ...
  else if(state === 'search') screen.innerHTML = shell('Check compatibility', `<button class="back" data-action="device-choice">← Change device</button><p class="screen-label">Another device</p><h2>Find the device you’ll use</h2><p class="sub">Compatibility is shown before you select a device.</p><input class="search" placeholder="Search iPhone, Samsung, Pixel…" autofocus /><div class="models"><button class="model supported-model" data-action="checkout"><b>iPhone 13</b><small>✓ Supports eSIM · Select</small></button><button class="model supported-model" data-action="checkout"><b>Samsung Galaxy S24</b><small>✓ Supports eSIM · Select</small></button><button class="model unsupported-model" disabled><b>Samsung Galaxy A14</b><small>✕ Doesn’t support eSIM</small></button><button class="model supported-model" data-action="checkout"><b>Google Pixel 9</b><small>✓ Supports eSIM · Select</small></button></div>`);
